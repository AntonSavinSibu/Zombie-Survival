...
MAX_ZOMBIES = 85
MAX_MINIONS = 45          # non-boss zombies
MAX_BULLETS = 120
MAX_ORBIT_BULLETS = 60
MAX_EXPLOSIONS = 24
MAX_POISON_CLOUDS = 18
MAX_MOON_WAVES = 20
MAX_MEDS = 20
...
# =========================================================
CHARACTER_PRICES = {"Survivor": 0, "Ninja": 900, "Engineer": 9500, "Time Lord": 2200, "Frozen": 3600, "Scientist": 5200, "Magician": 7500, "Reaper": 0}
CHARACTER_ABILITIES = {
    ...
    "Reaper": ("SHINIGAMI ARTS", "Q SENKAI TEMPEST  •  E GETSUGA JUJISHO  •  R BANKAI"),
}
CHARACTER_STATS = {
...
def load_profile():
    global coins, unlocked_characters, selected_character, character_index
...
def save_profile():
    try:
        with open(_account_save_path() if current_username else SAVE_FILE,"w",encoding="utf-8") as f:
            json.dump({"coins":coins,"unlocked":sorted(unlocked_characters),"selected":selected_character,
...
def buy_or_equip_class_skin():
    """Buy the highlighted class skin with coins, or equip it if already owned."""
    global coins, shop_flash, equipped_class_skins, owned_class_skins
    name = class_list()[character_index]
    ...
    if skin_owned(name, skin):
        ...
        equipped_class_skins[name] = skin["name"]
        shop_flash = (f"{skin['name']} UNLOCKED!", now)
    else:
        shop_flash = (f"NEED {skin['price']-coins:,} MORE COINS", now); return
    _PORTRAIT_CACHE.clear()
    save_profile()


def buy_or_equip_gun_skin():
    """Buy or equip the highlighted weapon skin in the armory."""
    global coins, shop_flash, equipped_gun_skin, owned_gun_skins
    skin = GUN_SKINS[gun_skin_index % len(GUN_SKINS)]
    now = pygame.time.get_ticks()
    if skin["key"] in owned_gun_skins or skin["price"] == 0:
...
def apply_upgrade(upgrade):
    ...
    if upgrade == "health":
        ...
        if is_melee_class():
            ...
            damage = int(damage * 1.5) + 1
            shoot_delay = min(shoot_delay, 58)
            bullet_speed = max(10, min(bullet_speed, 13))
        elif gun_level == 7:
...
def draw_player(surface, x, y, character_name=None):
    ...
    # Ground shadow
    shadow = pygame.Surface((70, 34), pygame.SRCALPHA)
    pygame.draw.ellipse(shadow, (0, 0, 0, 105), shadow.get_rect())
    surface.blit(shadow, shadow.get_rect(center=(x, y + 27)))
...
BLADE_TIER_NAMES = {1: "KATANA", 2: "TWIN KATANA", 3: "NODACHI", 4: "SPIRIT EDGE",
                    5: "CRESCENT FANG", 6: "DEMON FANG", 7: "DRAGON FANG", 8: "BANKAI BLADE"}
...
def _build_katana_art(level, skin_key=None):
    ...
    SS = 4
    W, H = {1: (104, 44), 2: (108, 58), 3: (120, 48)}.get(level, (132 + min(3, level - 5) * 6, 52))
    ...
    def blade(my, hilt_x, tip_x, thick, curve):
        # silhouette
        outline = curve_pts(my, hilt_x, tip_x, thick, curve, 1.1, True) + \
            list(reversed(curve_pts(my, hilt_x, tip_x, thick, curve, 1.1, False)))
        PGn(outline, (9, 10, 13))
        # steel body
        body = curve_pts(my, hilt_x, tip_x, thick, curve, 0.0, True) + \
            list(reversed(curve_pts(my, hilt_x, tip_x, thick, curve, 0.0, False)))
        PGn(body, SL)
        # shinogi: bright polished band along the upper half
    ...
    if level == 2:
        blade(H * 0.31, 40, W - 5, 4.2, 7)
        blade(H * 0.76, 40, W - 12, 4.2, 5)
        edges = [(H * 0.31, 4.2, 7)]
    else:
        my = H * 0.60
        thick = 5.2 if level <= 3 else 6.6
        curve = 9 if level <= 3 else 12
        blade(my, 40, W - 5, thick, curve)
        edges = [(my, thick, curve)]
        if level >= 6:
            # demon fang hooking off the spine near the tip
            ty = my - curve
            PGn([(W - 40, ty - thick + 2), (W - 12, ty - thick - 17), (W - 17, ty - thick + 3)], (9, 10, 13))
            PGn([(W - 38, ty - thick + 1), (W - 14, ty - thick - 14), (W - 20, ty - thick + 2)], SL)
            PGn([(W - 34, ty - thick), (W - 16, ty - thick - 11), (W - 22, ty - thick + 1)], EDGE)

    if level >= 4:
        # spirit energy burning along the cutting edge
        g = pygame.Surface((W * SS, H * SS), pygame.SRCALPHA)
        gcol = _cmix(AC, (255, 255, 255), 0.35)
        my, thick, curve = edges[0]
        pts = curve_pts(my, 40, W - 5, thick, curve, 0.0, True)
        for i in range(8, 0, -1):
            pygame.draw.lines(g, _sh(gcol, 0.16 * (i / 8.0) ** 2 + 0.02), False,
                              [(int(p[0] * k), int(p[1] * k)) for p in pts], max(1, int(i * 1.6 * k)))
        if level >= 7:
            for i, p in enumerate(pts[::3]):
                r = 1.3 + (i % 3) * 0.7
                pygame.draw.circle(g, _sh(gcol, 0.5), (int(p[0] * k), int((p[1] - 3 - i * 0.8) * k)),
                                   max(1, int(r * k)))
        s.blit(g, (0, 0), special_flags=pygame.BLEND_RGB_ADD)
...
BOSS_ROSTER = [
    ...
    {"key": "horde_king", "name": "HORDE KING",
     "hp": 120000, "speed": 1.05, "damage": 6.5, "radius": 56,
     "colors": ((255, 196, 40), (140, 90, 10), (255, 255, 160))},          # royal gold
    {"key": "plague_matriarch", "name": "PLAGUE MATRIARCH",
     "hp": 100000, "speed": 1.15, "damage": 5.0, "radius": 52,
     "colors": ((60, 255, 80), (15, 90, 25), (190, 255, 120))},             # toxic neon green
    {"key": "cyber_colossus", "name": "CYBER COLOSSUS",
     "hp": 150000, "speed": 0.85, "damage": 7.0, "radius": 60,
     "colors": ((40, 120, 200), (15, 40, 75), (0, 255, 255))},              # deep steel blue + cyan eyes
    {"key": "chrono_wraith", "name": "CHRONO WRAITH",
     "hp": 90000, "speed": 1.55, "damage": 5.5, "radius": 46,
     "colors": ((210, 90, 255), (55, 20, 100), (255, 230, 120))},           # violet body, gold eyes
    {"key": "blood_moon_reaper", "name": "BLOOD MOON REAPER",
     "hp": 110000, "speed": 1.35, "damage": 8.0, "radius": 50,
     "colors": ((190, 20, 35), (50, 5, 10), (255, 60, 60))},                # deep blood crimson
    {"key": "swarm_heart", "name": "SWARM HEART",
     "hp": 160000, "speed": 0.55, "damage": 4.5, "radius": 64,
     "colors": ((255, 40, 170), (100, 10, 70), (255, 200, 230))},           # hot pink / magenta
    {"key": "mirror_twin", "name": "MIRROR TWIN",
     "hp": 85000, "speed": 1.45, "damage": 6.0, "radius": 44,
     "colors": ((230, 240, 255), (110, 130, 160), (255, 255, 255))},        # chrome silver
    {"key": "frost_titan", "name": "FROST TITAN",
     "hp": 140000, "speed": 0.95, "damage": 6.5, "radius": 58,
     "colors": ((150, 230, 255), (40, 110, 170), (255, 255, 255))},         # ice blue / white
    {"key": "necro_conductor", "name": "NECRO CONDUCTOR",
     "hp": 105000, "speed": 1.10, "damage": 5.5, "radius": 52,
     "colors": ((90, 30, 140), (25, 8, 45), (255, 180, 40))},               # dark purple + amber eyes
]


def _make_named_boss(spec, x=None, y=None):
    if x is None or y is None:
        x, y = spawn_position()
    return {
        "x": float(x), "y": float(y),
        "speed": spec["speed"],
        "health": spec["hp"], "max_health": spec["hp"],
        "damage": spec["damage"], "radius": spec["radius"],
        "type": "boss", "boss_key": spec["key"], "boss_name": spec["name"],
        "colors": spec["colors"],
        "hit_flash_until": 0, "frozen_until": 0,
...
def spawn_wave():
    ...
    if wave % 5 == 0:
        ...
        for _ in range(bosses):
            x, y = spawn_position()
            hp = int((1250 + wave * 170) * (1.0 + wave * 0.02))
            zombies.append({
                "x": x, "y": y,
                "speed": 0.85 + wave * 0.030,
                "health": hp, "max_health": hp,
                "damage": 1.5 + wave * 0.03, "radius": 45, "type": "boss", "hit_flash_until": 0,
...
def shoot():
    ...
    if gun_level == 1:
        ...
        # Shotgun: exactly 2 pellets with a wider spread.
        angles = [base_angle - 0.14, base_angle + 0.14]
    elif gun_level == 6:
        # Minigun: a hail of rounds with a little barrel wander.
        angles = [base_angle + random.uniform(-0.10, 0.10)]
    elif gun_level == 7:
        # Flamethrower: a short, wide cone of burning fuel.
        angles = [base_angle - 0.21, base_angle, base_angle + 0.21]
    else:
        angles = [base_angle]
    ...
    for angle in angles:
        ...
        kind = {7: "flame", 8: "plasma"}.get(gun_level, "normal")
        bullet = {
            "x": sx,
            "y": sy,
        ...
        if kind == "flame":
            # Fuel loses steam quickly, so the flamethrower is short range.
            bullet["dx"] *= 0.66
            bullet["dy"] *= 0.66
            bullet["damage"] *= 0.55
...
def katana_slash(base_angle, arc=None, reach_bonus=0):
    """Reaper melee attack: a wide arc that cuts every zombie in front of the player."""
    now = pygame.time.get_ticks()
    reach = 118 + gun_level * 12 + reach_bonus
    half_arc = arc if arc else 0.85 + min(0.45, gun_level * 0.05)
    critical = random.random() < max(crit_chance, 0.18)
    hit_damage = damage * (crit_multiplier if critical else 1.0) * 1.15
    slashes.append({"x": float(player_x), "y": float(player_y), "angle": base_angle,
                    "reach": reach, "arc": half_arc, "time": now,
                    "critical": critical})
    hit_any = False
...
def kill_zombie(zombie):
    ...
    if zombie.get("boss_key"):
        # Named legendary bosses — huge payout
        score += 2500
...
MENU_BUTTONS = [
    (pygame.Rect(385, 238, 330, 44), "PLAY"),
    (pygame.Rect(385, 288, 330, 44), "MULTIPLAYER"),
    (pygame.Rect(385, 338, 330, 44), "CLASSES"),
    (pygame.Rect(385, 388, 330, 44), "ARMORY"),
    (pygame.Rect(385, 438, 330, 44), "LEADERBOARD"),
    (pygame.Rect(385, 488, 330, 44), "MODE"),
    (pygame.Rect(385, 538, 330, 44), "QUIT"),
]
...
ADMIN_POWERS = [
    ...
    {"key": "e", "name": "GETSUGA JUJISHO", "cd": 8000,
     "desc": "Fire a cross of giant piercing crescents that chain and explode on impact."},
...
def use_admin_power(key, current_time):
    """Reaper-only anime kit: Q Senkai Tempest, E Getsuga Jujisho, R Bankai."""
    global bankai_until, bankai_next, health
    if not (is_admin and selected_character == "Reaper"):
        return
    ...
    if not admin_power_ready(key, current_time):
        return
    power = next(p for p in ADMIN_POWERS if p["key"] == key)
    ...
    if key == "q":
        ...
        # GETSUGA JUJISHO — cross of four giant crescents + center beam
        mx, my = pygame.mouse.get_pos()
        angle = math.atan2(my - player_y, mx - player_x)
        # primary aimed fang
        for spread, spd, rad, dmg_mul, life in (
            (0.0, 18.0, 92.0, 3.4, 1800),
            (-0.42, 16.5, 78.0, 2.6, 1600),
            (0.42, 16.5, 78.0, 2.6, 1600),
            (math.pi, 14.0, 70.0, 2.2, 1400),  # rear slash for cross shape
        ):
            a = angle + spread
            moon_waves.append({
                "x": float(player_x), "y": float(player_y),
                "dx": math.cos(a) * spd, "dy": math.sin(a) * spd,
                "angle": a, "damage": damage * dmg_mul, "radius": rad,
                "time": current_time, "until": current_time + life, "hit": set(),
                "anime": True,
            })
        # delayed secondary crescents (afterimage style)
        for spread in (-0.22, 0.22):
            a = angle + spread
            moon_waves.append({
                "x": float(player_x - math.cos(angle) * 18),
                "y": float(player_y - math.sin(angle) * 18),
                "dx": math.cos(a) * 12.0, "dy": math.sin(a) * 12.0,
                "angle": a, "damage": damage * 1.8, "radius": 64.0,
                "time": current_time + 80, "until": current_time + 1500, "hit": set(),
                "anime": True, "delay": current_time + 120,
            })
...
def update_admin_powers(current_time):
    ...
    global player_x, player_y, bankai_until, bankai_next
...
def update_character_power(current_time):
    ...
    if is_admin and selected_character == "Reaper":
    ...
    if turret:
        if turret["health"] <= 0:
            ...
            target = min(zombies, key=lambda z: math.hypot(z["x"] - turret["x"], z["y"] - turret["y"]))
    ...
    if shadow_clone:
        if current_time >= shadow_clone["until"] or shadow_clone["health"] <= 0:
            ...
            target = min(zombies, key=lambda z: math.hypot(z["x"] - shadow_clone["x"], z["y"] - shadow_clone["y"]))
...
def reset_game():
    ...
    slashes.clear()
    moon_waves.clear()
    global bankai_until, bankai_next
    bankai_until = 0
    bankai_next = 0
...
def _admin_execute_command(command):
    ...
    global coins, unlocked_characters, selected_character
    ...
    if cmd == "help":
        return ("give money [n] | give admin [user] | give power | nocd | cooldown | "
                "money [n] | xp [n] | aimbot_on/off | unlock | kill | max | heal | freeze | boss")
    if cmd == "aimbot":
    ...
    if cmd == "give":
        ...
        if what in ("power", "powers", "reaper"):
            ...
            save_profile()
            return "GAVE ADMIN POWER: Reaper + max blades + no cooldown"
...
def admin_panel():
    ...
    while True:
        ...
        center_text(screen, FONT_TINY, "COMMANDS: help • aimbot_on/off • money • unlock • kill • xp • max • heal • freeze • boss • reset • reroll",
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # Character power
            if is_admin and selected_character == "Reaper" and event.key in (pygame.K_q, pygame.K_e, pygame.K_r):
                use_admin_power({pygame.K_q: "q", pygame.K_e: "e", pygame.K_r: "r"}[event.key], current_time)
                continue
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if game_over:
                restart_rect = pygame.Rect(WIDTH // 2 - 190, HEIGHT // 2 + 45, 380, 70)
                quit_rect = pygame.Rect(WIDTH // 2 - 150, HEIGHT // 2 + 125, 300, 65)
    ...
    if not intro_screen and not character_screen and not armory_screen and not paused and not game_over and not upgrade_screen and not gun_choice_screen and not mp_player_dead:
        ...
        if control_mode == "mobile":
            ...
            if mobile_ability:
                if is_admin and selected_character == "Reaper":
                    # cycle Q if ready else skip (R/E need keyboard still)
        ...
        if move_x or move_y:
            ...
            if length > 0:
                move_x /= length
                move_y /= length
            spd = player_speed * (0.45 if current_time < frost_slow_until else 1.0)
            player_x += move_x * spd
            player_y += move_y * spd
        ...
        # NEXT WAVE
        # -------------------------------------------------
        if len(zombies) == 0 and not upgrade_screen:
            wave += 1
    ...
    for zombie in zombies:
        ...
        # Fast path: crowded waves — simple circles for non-boss minions
        if _simple_draw and not is_named and ztype != "boss":
            pygame.draw.circle(screen, dark_c, (x, y), radius + 2)
            pygame.draw.circle(screen, body_c, (x, y), radius)
            if hit:
                pygame.draw.circle(screen, WHITE, (x, y), radius, 2)
            # tiny HP pip
            if zombie["health"] < zombie["max_health"]:
                bw = max(16, radius * 2)
                pygame.draw.rect(screen, RED, (x - bw // 2, y - radius - 8, bw, 3))
                pygame.draw.rect(screen, GREEN, (x - bw // 2, y - radius - 8,
                    int(bw * max(0, zombie["health"] / max(1, zombie["max_health"]))), 3))
            continue

        # Animated shadow + tiny bob
        ...
        screen.blit(shadow, shadow.get_rect(center=(x, y + radius // 2 + 12)))
    ...
    power_ready = False
    if is_admin and selected_character == "Reaper":
        # three-slot admin kit, each with its own cooldown
        for i, p in enumerate(ADMIN_POWERS):
        ...
        if game_over:
            ...
            if best_run_flags[0] or best_run_flags[1]:
                ...
                center_text(screen, FONT_SMALL, "NEW PERSONAL " + " + ".join(bl) + "!",
                            (WIDTH // 2, HEIGHT // 2 + 32), GOLD)

            restart_rect = pygame.Rect(WIDTH // 2 - 190, HEIGHT // 2 + 45, 380, 70)
            quit_rect = pygame.Rect(WIDTH // 2 - 150, HEIGHT // 2 + 125, 300, 65)
            mx, my = pygame.mouse.get_pos()
            draw_button(screen, restart_rect, "RESTART", restart_rect.collidepoint(mx, my))
            draw_button(screen, quit_rect, "QUIT", quit_rect.collidepoint(mx, my))
